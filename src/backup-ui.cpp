...
void print_usage() {
  std::cout << "Usage: backup-ui [OPTIONS] [COMMAND]\n\n"
            ...
            << "Options:\n"
            << "  -a, --average-size <SIZE>  Average chunk size in bytes "
               "(default: 8192)\n"
            << "  -t, --type <TYPE>         Backup type: full, incremental, "
               "differential (default: full)\n"
  ...
  try {
    if (command == "backup") {
      if (input_path.empty() || output_path.empty()) {
        std::cerr << "Error: Both input-path and output-path must be specified "
                     "for backup\n";
        return 1;
      }
      Backup backup(input_path, output_path, backup_type, remarks,
                    average_size);
      ...
      backup.DisplayAllBackupDetails(input_path);

    } else if (command == "compare") {
      if (input_path.empty() || argc < 4) {
        std::cerr << "Error: input-path and two backup names must be specified "
                     "for compare\n";
        return 1;
      }
      ...
      Backup backup(input_path, "", BackupType::FULL, "", average_size);
      backup.CompareBackups(input_path, backup1, backup2);
