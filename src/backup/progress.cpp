...
ProgressBar::ProgressBar(size_t total_size, size_t total_chunks,
  ...
  Logger::Log("Starting " + operation_name_ + " (" +
...
void ProgressBar::Update(size_t processed_bytes, size_t processed_chunks) {
  ...
  PrintProgress(progress, processed_bytes_, current_chunk_);
}

void ProgressBar::Complete() {
  // Print 100% progress
  total_chunks_ = std::max(current_chunk_, total_chunks_);
  PrintProgress(1.0f, total_size_, total_chunks_);
  Logger::Log("Completed " + operation_name_);
}

void ProgressBar::PrintProgress(float progress, size_t processed_bytes,
  ...
  oss << "] " << std::fixed << std::setprecision(1) << (progress * 100.0)
      << "% " << processed_bytes << "/" << total_size_ << " bytes ("
      << processed_chunks;
  ...
  auto minutes =
      std::chrono::duration_cast<std::chrono::minutes>(elapsed - hours);
