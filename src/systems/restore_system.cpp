...
void RestoreSystem::RestoreFromBackup() {
  try {
    ...
    // Get selected repository path - use the full path including repository
    // name
    std::string backup_path =
        repos[repo_choice - 1].path + "/" + repos[repo_choice - 1].name;
    ...
    if (location_choice == 0) {
      // For original location, calling the original path function to load
      // original path.
      std::string original_path =
          Restore::LoadOriginalPath(backup_path, backup_name);
      restore_dir =
          fs::path(original_path).parent_path() / ("restore_" + timestamp);
    } else {
    ...
    failed_log << "Failed Restores Log - " << TimeUtil::GetCurrentTimestamp()
               << "\n\n";
    success_log << "Successful Restores Log - "
    ...
    for (const auto& [file_path, metadata] : restore_op.GetMetadata().files) {
      ...
      for (const auto& chunk_hash : metadata.chunk_hashes) {
        ...
        fs::path chunk_path =
            fs::path(backup_path) / "chunks" / subdir / (chunk_hash + ".chunk");
      ...
      if (!missing_chunks.empty()) {
        failed_log << "Skipped restore (missing chunks): "
                   << metadata.original_filename << "\n";
        failed_log << "Original path: " << file_path << "\n";
        failed_log << "Missing chunks: " << missing_chunks.size() << "\n";
        failed_log << "Missing chunk hashes: "
                   << std::accumulate(
                          std::next(missing_chunks.begin()),
                          missing_chunks.end(), missing_chunks[0],
                          [](const std::string& a, const std::string& b) {
                            return a + ", " + b;
                          })
                   << "\n\n";
        failed_files.push_back(metadata.original_filename);
        Logger::Log("Skipped restore of " + metadata.original_filename +
                    " due to missing chunks");
        continue;
      }
      ...
      for (int attempt = 1; attempt <= 2 && !restore_success; attempt++) {
        try {
          ...
          if (attempt == 1) {
            Logger::Log("First attempt failed for file: " +
                        metadata.original_filename + " - Retrying...");
          }
        }
      }
      ...
      if (!restore_success) {
        ...
        Logger::Log("Failed to restore file after 2 attempts: " +
                    metadata.original_filename + " - " + error_message);
      }
    }
    ...
    if (failed_files.empty()) {
      if (location_choice == 0) {
        ...
        Logger::Log("Restore completed successfully to: " +
                    restore_dir.string());
      }
    } else {
      std::string summary =
          "Restore operation was unsuccessful. " +
...
void RestoreSystem::ResumeFailedRestore() {
  try {
    ...
    std::string restore_path =
        Prompter::PromptPath("Enter Path to Failed Restore Directory");
    ...
    if (!fs::exists(dumps_dir)) {
      ErrorUtil::ThrowError(
          "No dumps directory found. This may not be a failed restore "
          "directory.");
    }
    ...
    if (!fs::exists(failed_log_path)) {
      ErrorUtil::ThrowError(
          "No failed restores log found. This may not be a failed restore "
          "directory.");
    }
    ...
    while (std::getline(failed_log, line)) {
      if (line.find("Failed to restore: ") == 0) {
        failed_files.push_back(
            line.substr(19));  // Remove "Failed to restore: " prefix
      }
    }
    ...
    fs::path success_log_path =
        dumps_dir / "successful_files" / "successful_restores.log";
    ...
    if (backup_path.empty() || backup_name.empty()) {
      ErrorUtil::ThrowError(
          "Could not determine backup information from logs.");
    }
    ...
    if (still_failed_files.empty()) {
      Logger::Log(
          "Resume restore completed successfully. All previously failed files "
          "have been restored.");
