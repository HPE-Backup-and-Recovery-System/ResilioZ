...
DeleteScheduleDialog::DeleteScheduleDialog(QWidget* parent)
    : QDialog(parent), ui(new Ui::DeleteScheduleDialog) {
  ui->setupUi(this);
...
void DeleteScheduleDialog::checkSelection() {
  QModelIndexList selected =
      ui->schedule_table->selectionModel()->selectedRows();
...
void DeleteScheduleDialog::deleteSchedule(int row) {
  std::string schedule_id =
      ui->schedule_table->item(row, 0)->text().toStdString();
  ...
  ProgressBoxDecorator::runProgressBoxIndeterminate(
      ...
      [&](auto setWaitMessage, auto setSuccessMessage,
        ...
        try {
          ...
          Logger::SystemLog("Schedule " + schedule_id +
                            "deleted successfully.");
          setSuccessMessage("Schedule " + QString::fromStdString(schedule_id) +
                            " deleted successfully.");
          return true;
        } catch (const std::exception& e) {
          std::string issue = e.what();
          Logger::SystemLog("Schedule deletion failed: " + issue);
          setFailureMessage("Schedule deletion failed: " +
                            QString::fromStdString(issue));
          return false;
        }
      },
      "Deleting schedule...", "Schedule deleted successfully.",
