...
AttachScheduleDialog::AttachScheduleDialog(QWidget *parent,
    ...
    : QDialog(parent), ui(new Ui::AttachScheduleDialog) {
  ui->setupUi(this);
...
AttachScheduleDialog::~AttachScheduleDialog() { delete ui; }

std::string AttachScheduleDialog::getSchedule() { return cron_string; }
...
bool AttachScheduleDialog::handleSubmit() {
  ...
  if (cron_string_ == "" || !Validator::IsValidCronString(cron_string_)) {
    MessageBoxDecorator::showMessageBox(this, "Invalid Input",
                                        "Schedule CRON String is invalid.",
                                        QMessageBox::Warning);
...
void AttachScheduleDialog::on_backButton_clicked() { reject(); }
